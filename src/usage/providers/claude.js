      ...
      (error, stdout, stderr) => {
        if (error) {
          ...
          const msg = /could not be found|SecItemNotFound/i.test(stderr || "")
            ...
            : "钥匙串未授权（托盘菜单点“授权读取 Claude 余量”，弹窗选“始终允许”）";
...
// Re-trigger the macOS keychain prompt for "Claude Safe Storage" on demand, so a
// user who didn't click "Always Allow" the first time can grant it without
// reinstalling. Runs `security` directly with a long timeout (the user needs
// time to click), then clears the cached key + all throttle/backoff state so the
// next read fetches fresh. Returns { ok, reason }.
function requestKeychainAccess() {
    ...
    execFile(
      ...
      (error, _stdout, stderr) => {
        if (error) {
          resolve({
            ok: false,
            reason: /could not be found|SecItemNotFound/i.test(stderr || "")
              ? "未检测到 Claude 密钥（请先安装并登录 Claude 桌面端）"
              : "未授权（请在钥匙串弹窗点“始终允许 / Always Allow”）"
          });
          return;
        }
        // Authorized — reset throttle/backoff so the next refresh reads fresh.
        lastAttemptAt = 0;
        lastAttemptOk = false;
        nextAllowedAt = 0;
        consecutive429 = 0;
